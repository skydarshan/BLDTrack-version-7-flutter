...
class SiteStaffScreen extends StatefulWidget {
  const SiteStaffScreen({super.key});

  @override
  State<SiteStaffScreen> createState() => _SiteStaffScreenState();
}

class _SiteStaffScreenState extends State<SiteStaffScreen> {
  static const _permission = 'sitestaff';
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await context.read<SettingsApis>().siteStaff.list({
        'page': _page,
        'limit': 20,
        'search': _search,
      });
  ...
  Future<void> _openForm({Map<String, dynamic>? row}) async {
    ...
    final codeCtrl =
        TextEditingController(text: row?['employeeCode']?.toString() ?? '');
    final roleCtrl = TextEditingController(text: row?['role']?.toString() ?? '');
    final phoneCtrl =
            ...
            return Padding(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    Text(
                      isEdit ? 'Edit Site Staff' : 'Add Site Staff',
                    ...
                    FilledButton(
                      onPressed: () async {
                        ...
                        try {
                          final api = context.read<SettingsApis>().siteStaff;
                          if (isEdit) {
  ...
  Future<void> _delete(Map<String, dynamic> row) async {
    ...
    final ok = await confirmAction(
      context,
      title: 'Delete site staff',
      message: 'Delete "${row['name'] ?? 'staff'}"?',
      confirmLabel: 'Delete',
    );
    if (!ok) return;
    try {
      await context.read<SettingsApis>().siteStaff.delete(idOf(row)!);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Site Staff')),
                  ...
                  : _items.isEmpty
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (context, i) {
                            ...
                            return Card(
                              child: ListTile(
                                title: Text(row['name']?.toString() ?? 'Staff'),
                                subtitle: Text(
                                  [
